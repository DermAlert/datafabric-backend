...
class DatasetImageService:
    ...
    async def get_dataset_image_stats(self, dataset_id: int) -> DatasetImageStats:
        ...
        try:
            ...
            for obj in objects_generator:
                ...
                if any(obj_name_lower.endswith(ext) for ext in image_extensions):
                    ...
                    if obj.last_modified:
                        dates.append(obj.last_modified)
            
            # Calculate statistics
            latest_upload = max(dates) if dates else None
            oldest_upload = min(dates) if dates else None
            average_file_size = total_size / total_images if total_images > 0 else None
            ...
            logger.error(f"Error getting dataset image stats: {e}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=f"Error retrieving image statistics: {str(e)}"
            )
