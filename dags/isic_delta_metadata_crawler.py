...
def fetch_images_metadata(**context):
    """Fetch image metadata and save to a single partitioned Delta table"""
    ...
    # Introduce a counter for processed collections to allow for batch processing
    ...
    for idx, collection_row in collections_to_process.iterrows():
        ...
        try:
            ...
            all_images_metadata.extend(collection_images)
            total_images_processed += len(collection_images)
            processed_collections_count += 1
    ...
    # Write all collected images metadata to a single Delta table, partitioned by collection_id
    if all_images_metadata:
        df_all_images = pd.DataFrame(all_images_metadata)
        delta_path = get_delta_table_path(DELTA_BUCKET, IMAGES_METADATA_DELTA_PATH)
        
        try:
            # Use the safer schema-aware write function with partitioning
            success = safe_write_with_schema_evolution(
                df_all_images,
                delta_path,
                mode="overwrite", # Overwrite the entire table, or use "append" with merge logic
                partition_by=['collection_id']
            )
            
            if success:
                logger.info(f"Successfully wrote {len(df_all_images)} images to {IMAGES_METADATA_DELTA_PATH} partitioned by collection_id")
            else:
                logger.error(f"Failed to write images to {IMAGES_METADATA_DELTA_PATH}")
        
        except Exception as e:
            logger.error(f"Error writing Delta table {IMAGES_METADATA_DELTA_PATH}: {str(e)}")
            raise
    else:
        logger.info("No image metadata to write.")

    logger.info(f"Total images processed: {total_images_processed}")
    
    return {
        "total_images": total_images_processed,
        "collections_processed": processed_collections_count,
        "timestamp": timestamp
    }
...
def download_images_batch(**context):
    ...
    try:
        # Read the entire partitioned Delta table
        if not delta_table_exists(base_path):
            logger.info(f"Delta table does not exist at {base_path}, skipping download.")
            return {
                "downloaded_count": 0,
                "error_count": 0,
        ...
        # Group by collection_id to respect MAX_IMAGES_PER_COLLECTION if set
        for collection_id, collection_df in df_all_images.groupby('collection_id'):
            logger.info(f"Processing {len(collection_df)} images from collection {collection_id}")
            ...
            logger.info(f"Completed collection {collection_id}: Downloaded={images_downloaded_for_collection}, Skipped={images_skipped_for_collection}, Available={len(collection_df)}")
            total_images_skipped += images_skipped_for_collection
        
        total_collections_processed = len(df_all_images['collection_id'].unique()) if not df_all_images.empty else 0
        total_images_available = len(df_all_images)
...
fetch_images_metadata_task = PythonOperator(
    ...
    retries=2,
    retry_delay=timedelta(minutes=5),
    # Increase the execution timeout for this task as it can be long-running
    execution_timeout=timedelta(hours=4), # Increased from 2 hours to 4 hours
)
